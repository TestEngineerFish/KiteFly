...
/// 录入语音页面
class KFRecordToolView: KFTopWindowView, KFAudioManagerDelegate {
    ...
    /// 音浪气泡容器
    private var bubbleImageview: KFImageView = {
    ...
    /// 音浪气泡动画
    private var audioAnimationLayer = KFRecordVoiceAnimationLayer(number: 26, height: AdaptSize(25), isCancel: false)
    /// 取消时展示的气泡
    private var cancelVoiceAnimationLayer = KFRecordVoiceAnimationLayer(number: 10, height: AdaptSize(25), isCancel: true)
    ...
    private var comitButton: KFButton = {
        let button = KFButton()
    ...
    /// 按下区域
    private lazy var recordAreaView: KFView = {
        let view = KFView()
    ...
    private var recordImageView: KFImageView = {
        let imageView = KFImageView()
    ...
    deinit {
        KFAudioManager.share.delegate = nil
    }
    ...
    internal override func show(view: UIView = kWindow) {
        ...
        KFAudioManager.share.delegate = self
        KFAudioManager.share.startRecording()
    }
    ...
    internal override func hide() {
        ...
        self.timer = nil
        KFAudioManager.share.stopRecording()
        super.hide()
    }
