...
protocol BPChatInputViewDelegate: NSObjectProtocol {
...
class BPChatInputView:
    BPView,
    UITextViewDelegate,
    BPRecordToolViewDelegate {
    ...
    weak var delegate: BPChatInputViewDelegate?
    
    private var recordIconButton: BPButton = {
        let button = BPButton(size: CGSize(width: AdaptSize(33), height: AdaptSize(33)))
    ...
    private var recordBtton: BPButton = {
        let button = BPButton()
...
//        button.setImage(UIImage(named: "chat_tools_more_selected"), for: .selected)
    ...
    /// 录入语音页面
    private var recordToolView: BPRecordToolView?
    ...
    ///  编辑、录音切换
    @objc
    private func clickRecordIconButton(sender: BPButton) {
    ...
    /// 表情
    @objc
    private func emojiAction(sender: BPButton) {
        sender.isSelected = !sender.isSelected
        if sender.isSelected {
            self.recordBtton.isHidden   = true
            self.textView.isHidden      = false
    ...
    @objc
    private func moreAction(sender: BPButton) {
        sender.isSelected = !sender.isSelected
        if sender.isSelected {
            self.recordBtton.isSelected = false
            self.emojiButton.isSelected = false
    ...
    private func panAction(sender: UIPanGestureRecognizer) {
        if self.recordToolView == nil {
            self.recordToolView = BPRecordToolView()
