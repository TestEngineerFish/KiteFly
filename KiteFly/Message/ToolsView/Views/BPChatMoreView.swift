...
protocol BPChatMoreViewDelegate: NSObjectProtocol {
...
class BPChatMoreView: BPView {
    
    weak var delegate: BPChatMoreViewDelegate?
    
    /// 常用语
    private var commonWordsButton: BPButton = {
        let button = BPButton(size: CGSize(width: AdaptSize(50), height: AdaptSize(50)))
        button.setImage(UIImage(named: "chat_tools_commonWords"), for: .normal)
        return button
    }()
    
    /// 音视频
    private var videoButton: BPButton = {
        let button = BPButton(size: CGSize(width: AdaptSize(50), height: AdaptSize(50)))
    ...
    /// 照片
    private var imageButton: BPButton = {
        let button = BPButton(size: CGSize(width: AdaptSize(50), height: AdaptSize(50)))
    ...
    override func createSubviews() {
        ...
        let stackView = BPStackView(type: .center, subview: [commonWordsButton, videoButton, imageButton, giftButton], spacing: AdaptSize(32))
