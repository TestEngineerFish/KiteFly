...
protocol BPSystemPhotoViewDelegate: NSObjectProtocol {
    func clickImage(indexPath: IndexPath, from imageView: UIImageView?)
    func selectedImage()
    func unselectImage()
}

class BPSystemPhotoView: BPView, UICollectionViewDelegate, UICollectionViewDataSource, BPPhotoAlbumCellDelegate {
    
    weak var delegate: BPSystemPhotoViewDelegate?
    ...
    func reload(album model: BPPhotoAlbumModel?) {
