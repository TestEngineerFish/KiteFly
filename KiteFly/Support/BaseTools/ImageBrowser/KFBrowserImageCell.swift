...
protocol KFBrowserImageCellDelegate: NSObjectProtocol {
...
class KFBrowserImageCell: KFCollectionViewCell, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    weak var delegate: KFBrowserImageCellDelegate?
    ...
    private var progressView = KFProgressView(type: .round, size: CGSize(width: AdaptSize(60), height: AdaptSize(60)), lineWidth: AdaptSize(5))
...
//            }
    ...
    private func getOriginImage(model: BPMediaImageModel, progress: ((CGFloat) ->Void)?, completion: ((UIImage?)->Void)?, userId: String) {
        ...
        if let _sessionId = model.sessionId, _sessionId.isNotEmpty,
           let imageData = KFFileManager.share.getSessionMedia(type: .sessionImage, name: model.originLocalPath!, sessionId: _sessionId, userId: userId){
            // 如果是聊天室查看
            ...
            if let _path = model.originLocalPath, let image = UIImage(contentsOfFile: _path) {
                ...
                KFDownloadManager.share.image(urlStr: path, progress: progress, completion: completion)
            } else {
                completion?(nil)
            }
        }
