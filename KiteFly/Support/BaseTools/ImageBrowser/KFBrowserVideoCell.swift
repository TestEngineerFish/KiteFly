...
protocol KFBrowserVideoCellDelegate: NSObjectProtocol {
...
class KFBrowserVideoCell:
    KFCollectionViewCell,
    KFVideoManagerDelegate,
    UIGestureRecognizerDelegate {
    
    weak var delegate: KFBrowserVideoCellDelegate?
    private let playManager = KFVideoManager()
    ...
    private var leftTimeLabel: KFLabel = {
        let label = KFLabel()
        label.text          = "00:00"
    ...
    private var rightTimeLabel: KFLabel = {
        let label = KFLabel()
        label.text          = "00:00"
        label.textColor     = UIColor.white
        label.font          = UIFont.regularFont(ofSize: AdaptSize(12))
    ...
    private var progressView = KFProgressView(type: .line, size: CGSize(width: AdaptSize(250), height: AdaptSize(2)))
    
    private var closedButton: KFButton = {
        let button = KFButton()
    ...
    @objc private func playAction(sender: KFButton) {
