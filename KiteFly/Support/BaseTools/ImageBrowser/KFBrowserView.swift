...
public enum KFImageBrowserType {
    /// 自定义
...
public protocol KFImageBrowserDelegate:NSObjectProtocol {
    func reuploadImage(model:[BPMediaModel])
}

public class KFBrowserView:
    KFView,
    UICollectionViewDelegate,
    UICollectionViewDataSource,
    KFBrowserImageCellDelegate,
    KFBrowserVideoCellDelegate {
    
    public weak var delegate:KFImageBrowserDelegate?
    ...
    private var type: KFImageBrowserType
...
//        layout.minimumInteritemSpacing = AdaptSize(20)
    ...
    private var albumButton: KFButton = {
        let button = KFButton()
    ...
    public init(type: KFImageBrowserType, current index: Int) {
        self.type         = type
    ...
    public override func bindProperty() {
        ...
        self.collectionView.register(KFBrowserImageCell.classForCoder(), forCellWithReuseIdentifier: kBPBrowserImageCellID)
        self.collectionView.register(KFBrowserVideoCell.classForCoder(), forCellWithReuseIdentifier: kBPBrowserVideoCellID)
    ...
    private func showAlubmVC() {
        let vc = KFPhotoAlbumViewController()
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .custom(let modelList):
            ...
            switch model.type {
            case .image:
                guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kBPBrowserImageCellID, for: indexPath) as? KFBrowserImageCell, let imageModel = model as? BPMediaImageModel else {
                ...
                return cell
            case .video:
                guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kBPBrowserVideoCellID, for: indexPath) as? KFBrowserVideoCell, let videoModel = model as? BPMediaVideoModel else {
                    return KFCollectionViewCell()
                }
                ...
                return cell
            default:
                return KFBrowserImageCell()
            }
        case .system(let result):
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kBPBrowserImageCellID, for: indexPath) as? KFBrowserImageCell else {
                return KFCollectionViewCell()
            }
            cell.delegate = self
    ...
    public func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        (cell as? KFBrowserImageCell)?.updateZoomScale()
    }
    ...
    public func longPressAction(image: UIImage?) {
        KFActionSheet().addItem(title: "保存") {
