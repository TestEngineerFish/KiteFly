...
public class BPDatabaseManager {
    ...
    /// - Returns: FMDB数据库执行器
    public func getRunner(type: BPDatabaseType) -> FMDatabase {
        return try! normalRunner()
    }
    
    public func getQueueRunner(type: BPDatabaseType) -> FMDatabaseQueue? {
    ...
    private func normalRunner() throws -> FMDatabase {
        if let runner = self.runner {
            ...
            let filePath = self.dbFilePath(fileName: BPDatabaseType.normal.dbName)
    ...
    private func normalQueueRunner() throws -> FMDatabaseQueue? {
        if let runner = self.queueRunner {
            ...
            let filePath = self.dbFilePath(fileName: BPDatabaseType.normal.dbName)
            return try createQueueRunner(type: .normal, filePath: filePath, sqls: BPSQLManager.createNormalTables)
        }
    }
    ...
    private func createQueueRunner(type: BPDatabaseType, filePath: String, sqls: [String]) throws -> FMDatabaseQueue? {
