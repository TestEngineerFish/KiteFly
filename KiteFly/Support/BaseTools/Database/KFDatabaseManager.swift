...
public enum KFDatabaseType: String {
    /// 普通数据库
    case normal = "normal"
    /// 数据库本地名称
    var dbName: String {
        switch self {
        case .normal:
            return "001_\(self.rawValue).db"
        }
    }
}

public enum KFSQLError: Error {
    case CreateDatabaseError
    case CreateTableError
    case ExecuteSQLError
}

public class KFDatabaseManager {

    /// FMDB全局单例
    public static let `default` = KFDatabaseManager()

    /// 存储所有的执行器
    private var runner: FMDatabase?
    private var queueRunner: FMDatabaseQueue?

    /// 根据数据源类型, 返回对应的执行器
    ///
    /// - Parameter type: 数据源类型
    /// - Returns: FMDB数据库执行器
    public func getRunner(type: KFDatabaseType) -> FMDatabase {
        return try! normalRunner()
    }
    
    public func getQueueRunner(type: KFDatabaseType) -> FMDatabaseQueue? {
        return try! normalQueueRunner()
    }
    ...
    private func normalRunner() throws -> FMDatabase {
        if let runner = self.runner {
            ...
            let filePath = self.dbFilePath(fileName: KFDatabaseType.normal.dbName)
            return try createRunner(type: .normal, filePath: filePath, sqls: KFSQLManager.createNormalTables)
        }
    }
    
    private func normalQueueRunner() throws -> FMDatabaseQueue? {
        if let runner = self.queueRunner {
            return runner
        } else {
            let filePath = self.dbFilePath(fileName: KFDatabaseType.normal.dbName)
            return try createQueueRunner(type: .normal, filePath: filePath, sqls: KFSQLManager.createNormalTables)
        }
    }
    ...
    /// - Returns: 数据库执行器
    /// - Throws: 创建数据异常
    private func createRunner(type: KFDatabaseType, filePath: String, sqls: [String]) throws -> FMDatabase {
        ...
        guard runner.open() else {
            ...
            throw KFSQLError.CreateDatabaseError
        }
        for sql in sqls {
    ...
    private func createQueueRunner(type: KFDatabaseType, filePath: String, sqls: [String]) throws -> FMDatabaseQueue? {
        let runner = FMDatabaseQueue(path: filePath)
        for sql in sqls {
            runner?.inDatabase({ db in
                let success = db.executeStatements(sql)
                if !success {
                    print(String(format: "Error: execute sql: %@, failed error message: %@", sql, db.lastErrorMessage()))
                }
            })
        }
        return runner
