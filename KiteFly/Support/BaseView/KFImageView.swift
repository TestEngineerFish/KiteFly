...
open class KFImageView: UIImageView, KFViewDelegate {
                    ...
                    self.sd_setImage(with: imageURL, placeholderImage: nil, options: .lowPriority, progress: nil) { _image, error, cacheType, url in
                        if let _ = error {
                            ...
                            DispatchQueue.global().async {
                                ...
                                KFFileManager.share.saveSessionMedia(type: .sessionImage, name: _urlStr, sessionId: _sessionId, data: _data, userId: "")
                            }
                            block?(_image)
                        }
                    }
    ...
    func getLocalImage(name: String?, sessionId: String?) -> UIImage? {
        ...
        guard let data = KFFileManager.share.getSessionMedia(type: .sessionImage, name: _name, sessionId: _sessionId, userId: "") else { return nil }
