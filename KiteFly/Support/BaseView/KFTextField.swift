...
open class KFTextField: UITextField, UITextFieldDelegate, KFViewDelegate {
    
    private var type: KFTextFieldType
    /// 最长字符长度
    ...
    public var showLeftView: Bool = true {
        willSet {
            if newValue {
                let _leftView = KFView(frame: CGRect(origin: .zero, size: CGSize(width: AdaptSize(15), height: AdaptSize(1))))
    ...
    public var showRightView: Bool = false {
        willSet {
            if newValue {
                let _rightView = KFView(frame: CGRect(origin: .zero, size: CGSize(width: AdaptSize(15), height: AdaptSize(1))))
    ...
    public init(type: KFTextFieldType = .normal) {
        self.type = type
    ...
    public func updateType(type: KFTextFieldType) {
        self.type = type
    }
